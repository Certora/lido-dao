...
contract StakingRouter is AccessControlEnumerable, BeaconChainDepositor, Versioned {
    ...
    function deposit(
        ...
        if (_depositsCount > 0) {
            ...
            // _makeBeaconChainDeposits32ETH(                      // munge payable(address(DEPOSIT_CONTRACT)).transfer(DEPOSIT_SIZE);
            //     _depositsCount,                                  // comment out memUtils
            //     abi.encodePacked(withdrawalCredentials),        // rerun sanity
            //     publicKeysBatch,
            //     signaturesBatch
            // );
            for (uint256 i; i < _depositsCount;) {
                // Receiver(payable(DEPOSIT_CONTRACT)).sendTo{value:DEPOSIT_SIZE}();
